...
- 可处理范围：创建预约指引、修改预约指引、取消预约指引、分批到仓处理、预约状态查询、违规费说明 / 查询、预约 POD 下载指引。
- 不可处理范围：代客创建 / 修改 / 取消预约、实时 slot 查询、PDF 下载、库容 / 额度、入库单总状态、签收轨迹、VASC 推荐、服务项配置、已提交增值单状态。
- 候选范围的实质是“可输出的服务类型集合”，不是可执行写操作集合。
