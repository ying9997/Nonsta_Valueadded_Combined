...
function mergePluginOutputs(params: Record<string, unknown>): Record<string, unknown> {
  ...
  return { list, total: list.length, _fetchMeta: { strategy: "detail-only", batchPluginMerged: true } };
}
...
async function main({ params }: { params: Record<string, unknown> }) {
  ...
  if (!env) {
    return {
      rawOrderData: { list: [], total: 0, _fetchMeta: { strategy: "local-fetch-skipped", reason: "missing Coze env" } },
    };
  }
  ...
  return { rawOrderData: { list, total: list.length, _fetchMeta: { strategy: "local-proxy-detail" } } };
}
