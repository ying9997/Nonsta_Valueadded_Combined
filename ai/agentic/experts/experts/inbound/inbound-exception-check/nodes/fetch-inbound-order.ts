...
function mergePluginOutputs(params: Record<string, unknown>): Record<string, unknown> {
  ...
  return { list, total: list.length, _fetchMeta: { strategy: "detail-batch", resolvedCount: list.length } };
}

async function main({ params }: { params: Record<string, unknown> }) {
  if (params.skipDetailApi === true) {
    return { rawOrderData: { list: [], total: 0, _fetchMeta: { strategy: "list-mode-skip-detail" } } };
  }
  ...
  if (!env) {
    return { rawOrderData: { list: [], total: 0, _fetchMeta: { strategy: "skipped", reason: "missing Coze env" } } };
  }
  ...
  return {
    rawOrderData: {
      list,
      total: list.length,
      _fetchMeta: { strategy: "local-proxy", resolvedCount: list.length, errors: errors.length ? errors : undefined },
    },
  };
}
