 ...
 *
 * **Prompt**：`prompts/main.md`（占位符含 `{{compensateListFacts}}`、`{{kbMd}}` 等，见 `scripts/llm-openai.ts`）。
 *
 * 【输入】
 * | 字段 | 说明 |
 * |------|------|
 * | query / customerIntent | 委托与意图 |
 * | trackingIds / outboundOrderNos / claimIds | 归一化后的查询键 |
 * | enrichedContext | 可选；上游 delivery-status 等 |
 * | inputContext | 可选；含 previousOutput |
 * | compensateListFacts | **fetch-compensate-list** 产出的确定性事实（JSON） |
 *
