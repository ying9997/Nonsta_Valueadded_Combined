 ...
 * | productLine | string | `{{productLine}}`（psc/wf/pallet/""） |
 * | matchedProductNames | string[] | `{{matchedProductNames}}`（validate-input 匹配到的产品名） |
 * | kbContent | string | `{{kbContent}}`（load-product-knowledge 过滤后的聚焦 KB） |
 ...
 * | 字段 | 类型 | 说明 |
 * |------|------|------|
 * | analysisResult | { structured: object; analysis: string } | 产品信息 JSON，供 `format-output.ts` |
 */
