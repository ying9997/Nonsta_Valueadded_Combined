# outbound-order-status 设计参考

> 域：`outbound` · 专家 ID：`outbound/outbound-order-status` · 实现见 `experts/outbound/outbound-order-status/`
...
| **shipping-label** | 查询并下载仅供查看/申诉的尾程面单；不可用于实际发货打印 |
