...
## 业务场景
根据多维度场景（目的国、产品、事件类型等）从规则库匹配赔付条款，解读赔偿规则、赔付上限、理算逻辑与免责范围。权威来源是飞书表格《WINIT 赔付标准》，数据同步到专家知识库。不负责索赔单进度查询/申请流程，该项由 `claim` 专家承接。
...
| **不适用场景** | 索赔入口、材料提交、进度查询 → 转 `claim` 专家 |
...
| `structured.suggestedNextStep` | string | 建议下游：`route_to_claim`/`need_order_details`/`escalate_human` |
