...
fidelity: summary
status: draft
service_item_code: OW01V1604
service_item_name: 上架前自提（需WINIT打托）
service_item_aliases: [增值原子, 增值事件, 上架前自提, 托盘自提, 需打托自提]
service_item_object_level: package
service_item_type: non_standard
service_item_required_in_vasc: false
service_item_mutex_group: 上架前自提-托盘
...
`上架前自提（需WINIT打托）` 是入库异常链路中的自提原子，用于异常货物到海外仓后，客户要求 Winit 先将货物打托，再由客户货代提走的场景。

本原子与 `上架前自提（无需WINIT打托）` 的核心差异是是否需要 Winit 打托。若客户只是按包裹提货，不需要仓库打托，应选择无需打托自提。
...
|---|---|
| 服务项编码 | `OW01V1604` |
| 服务项名称 | 上架前自提（需WINIT打托） |
| 别名 | 增值原子 / 增值事件 / 托盘自提 / 需打托自提 |
| PSCG | `OW01` 海外仓入库 |
| 操作对象 | 包裹 |
| 是否原子增值 | Y |
| 是否拦截入库列表 | Y |
...
仓库动作是对异常暂存或上架前的包裹执行 Winit 打托准备，使货物以托盘形态供客户货代提走。执行完成后，实物退出入库上架链路。

若客户同时需要 Winit 协助补贴快递面单，业务 SOP 提示可上传快递面单到增值文件；但当前字段证据未展开具体附件字段，本页不定版字段名、文件格式或必填条件。

## 所属 VASC 产品

| VASC 产品 | 编码 | 顺序 | 产品级必选 | 互斥组 | 说明 |
|---|---|---:|---|---|---|
| 上架前自提 | `VASC202411192240522` | 2 | N | 上架前自提-托盘 | 托盘自提或需要 Winit 打托后提货时选择。 |

## 核心适用场景

| 场景 | 是否可考虑 | 判断依据 |
|---|---|---|
| 异常货物到仓后，客户要求打托后提货 | 是 | 主数据定义为货物到海外仓后客户要求打托后提货。 |
| 托盘自提 | 是 | SOP 明确托盘自提选需 Winit 打托。 |
| 客户只按包裹提货，不需要打托 | 不应选 | 应选择 `上架前自提（无需WINIT打托）`。 |
| 客户要求销毁 | 不应选 | 应查上架前销毁原子。 |
| 客户要求继续上架 | 不应选 | 应查原单/新单/直接上架原子。 |

## 配置字段

当前字段覆盖映射显示 `OW01V1604` 为 `missing_field_evidence`，normalized 和字段快照没有提供可定版的 `attrSpec` 字段。

AI 回答字段配置时，应说明：当前知识库没有足够字段级证据确认本原子的具体必填字段、附件字段、打托参数字段或模板字段。
...
| 需要 Winit 打托的确认 | 所有场景 | 主数据和 SOP 均明确本原子是打托后提货。 |
...
- 必须确认需要 Winit 打托或托盘自提。
- 如只是包裹自提且不需要打托，应选择 `上架前自提（无需WINIT打托）`。
...
| 原子 | 区别 |
|---|---|
| 上架前自提（无需WINIT打托） | 包裹自提，无需 Winit 打托；本页是打托后提货。 |
| 上架前包裹销毁 | 实物由仓库销毁；本页是客户货代提走。 |
| 直接上架 | 实物继续入库上架；本页实物退出上架链路。 |

## 证据边界

- 本页不生成确定字段清单、打托参数、附件格式、面单模板、预约字段和提货时段字段。
- 本页不定版具体费用金额、最低收费金额、国家仓库差异。
- 面单上传只作为 SOP 操作提示，不作为 `attrSpec` 字段证据。

## 相关链接

- [上架前自提（无需WINIT打托）](value-added-service-item-pre-putaway-self-pickup-without-winit-palletizing.md)
