---
title: wh.va.order.getPrepaymentList — 查询增值预付款列表
type: reference
entity_type: interface_reference
tags: [interface-reference, value-added-service, reference]
source_refs: ["source-references/interface-documents/wh-va-order-get-prepayment-list-api.md"]
updated: 2026-06-22
confidence: medium
fidelity: preserve
status: active
---
# wh.va.order.getPrepaymentList — 查询增值预付款列表

## 接口概览

| 项目 | 说明 |
|------|------|
| 接口名称 | `wh.va.order.getPrepaymentList` |
| 接口路径 | `POST /wh/va/order/getPrepaymentList` |
| 接口描述 | 查询指定增值订单的预估费用信息，包含各增值原子的应收及成本明细 |
...
| 字段 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| action | String | 是 | - | 固定值 `wh.va.order.getPrepaymentList` |
...
| costTotalStandardCurrencyCode | String | 预估成本本位币币种 |
| costTotalStandardAmount | BigDecimal | 预估成本本位币金额 |
| atomFeeList | List\<VaAtomFeeEstimateVo\> | 各增值原子费用预估列表，见下 |
| feeList | List\<InHouseOrderRevenueVo\> | 入库相关费用列表，见下 |

### VaAtomFeeEstimateVo — 原子费用预估
