...
flowchart TD
    ...
    C1 -->|最多1次追问；超过则用默认值继续| B
    D -->|可先用合理默认值规划| E["3. 任务拆解"]
    E --> F["4. 推理质量检查"]
    F --> G{"规划链是否自洽？"}
    G -->|否；最多2次，超过则节点FAIL| E
    G -->|是| H["5. 工具调用"]
    H --> I["6. 多步执行与状态管理"]
    I --> J{"工具/数据/约束是否异常？"}
    J -->|异常| K["7. 异常恢复"]
    K -->|最多3次，超过则异常恢复FAIL| I
    J -->|正常| L["8. 幻觉检测"]
    L --> M{"事实是否可验证？"}
    M -->|存在疑似幻觉；最多2次，超过则幻觉检测FAIL| H
    M -->|通过| N["9. 最终输出"]
    N --> O["端到端基线对比"]
```
...
自动化评估覆盖目标：V1.1中可由规则、脚本、正则、日志或计数器直接判定的指标占比应≥50%。人工评审工时按完整用例估算：每个端到端用例约8-12分钟，其中事实核验与行程可执行性判断约5-7分钟，主观评分与问题记录约3-5分钟。

### 0.4 阈值校准方法论

量化阈值分为两类：

- 硬约束：业务底线或安全底线，不通过pilot run下调。例如相对日期解析准确率=100%、景点真实性准确率=100%、严重幻觉数=0、P0一票否决=0、默认假设声明率=100%。硬约束来自任务正确性、安全性和用户信任底线，不需要校准。
- 软约束：用于衡量质量、体验、覆盖和效率，可通过pilot run校准。例如子任务覆盖率≥90%、可读性评分≥4分、行程可执行率≥90%、工具调用次数≤10次、端到端完成耗时P95≤30秒。软约束是V1.1初始值，需用实际运行分布确认是否合理。

校准流程：

1. 使用冻结评测输入集执行5-10次pilot run，覆盖核心输入和主要变体。
2. 收集每个指标的实际分布，包括均值、P25、P75、最小值、最大值和异常样本。
3. 对正向指标，将阈值设定在P25附近，使约75%的正常运行能通过；对负向指标，如延迟、调用次数、无效调用率，将阈值设定在P75附近，使约75%的正常运行能通过。
4. 硬约束不得因pilot表现不佳而放宽；若硬约束未达标，应修复Agent或评测数据。
5. 校准后更新文档版本，例如V1.1-calibrated或V1.2，并在变更记录中说明调整原因、pilot样本量和生效范围。

V1.1中所有非硬约束阈值均标记为“初始值待校准”。校准后应将“建议阈值/通过阈值”更新为“校准后阈值”，并保留原始阈值作为历史记录。

校准记录表模板：

| 指标 | pilot均值 | pilot P25 | pilot P75 | 当前阈值 | 校准后阈值 | 调整理由 |
|---|---:|---:|---:|---:|---:|---|
| 子任务覆盖率 | 待填 | 待填 | 待填 | ≥90% | 待填 | 待填 |
| 行程可执行率 | 待填 | 待填 | 待填 | ≥90% | 待填 | 待填 |
| 可读性评分 | 待填 | 待填 | 待填 | ≥4/5 | 待填 | 待填 |
| 端到端完成耗时P95 | 待填 | 待填 | 待填 | ≤30秒 | 待填 | 待填 |
| 工具调用次数 | 待填 | 待填 | 待填 | ≤10次 | 待填 | 待填 |
...
- 回归测试最小样本量：版本迭代后至少重跑n≥10次，确认无显著退化。

统计口径：

- 连续型指标报告均值±95%置信区间，例如延迟、token消耗、工具调用次数、人工评分。
- 比率型指标使用Wilson区间法计算95%置信区间，例如准确率、成功率、覆盖率、通过率。
...
| 指标 | 分子 | 分母 | 通过阈值 | 评估方式 |
|---|---:|---:|---:|---|
| 任务类型识别准确率 | 正确识别为旅行规划的用例数 | 意图理解用例总数 | 100% | 自动 |
| 核心实体抽取准确率 | 正确抽取的核心实体数 | 应抽取核心实体数 | ≥95%（初始值待校准） | 自动 |
| 相对日期解析准确率 | 正确解析相对日期的用例数 | 含相对日期用例数 | 100% | 自动 |
| 关键缺失信息识别率 | 正确识别缺失关键信息的项数 | 标注缺失关键信息总项数 | ≥90%（初始值待校准） | 自动 |
...
| 指标 | 分子 | 分母 | 通过阈值 | 评估方式 |
|---|---:|---:|---:|---|
| 主动澄清触发率 | 正确主动追问的缺失关键信息用例数 | 需澄清用例总数 | ≥90%（初始值待校准） | 半自动 |
| 澄清问题有效率 | 问题能直接减少规划不确定性的数量 | Agent提出的问题总数 | ≥85%（初始值待校准） | 人工 |
| 澄清问题简洁率 | 单轮追问不超过5个关键问题的用例数 | 需追问用例总数 | ≥95%（初始值待校准） | 自动 |
| 默认假设声明率 | 使用默认假设且明确声明的用例数 | 使用默认假设用例总数 | 100% | 自动 |
...
| 指标 | 分子 | 分母 | 通过阈值 | 评估方式 |
|---|---:|---:|---:|---|
| 子任务覆盖率 | 正确覆盖的必要子任务数 | 标注必要子任务总数 | ≥90%（初始值待校准） | 半自动 |
| 依赖顺序合理率 | 依赖顺序合理的用例数 | 任务拆解用例总数 | ≥90%（初始值待校准） | 人工 |
| 约束映射完整率 | 用户约束被映射到子任务的项数 | 用户约束总项数 | ≥90%（初始值待校准） | 半自动 |
| 任务粒度合格率 | 子任务既不遗漏也不过度碎片化的用例数 | 任务拆解用例总数 | ≥85%（初始值待校准） | 人工 |
...
| 指标 | 分子 | 分母 | 通过阈值 | 评估方式 |
|---|---:|---:|---:|---|
| 推理链自洽率 | 未出现前后矛盾的规划链用例数 | 推理质量用例总数 | ≥95%（初始值待校准） | 人工 |
| 执行顺序合理率 | 工具调用前规划顺序合理的用例数 | 推理质量用例总数 | ≥90%（初始值待校准） | 人工 |
| 约束冲突识别率 | 正确发现约束冲突的数量 | 标注约束冲突总数 | ≥90%（初始值待校准） | 半自动 |
| 假设标注准确率 | 明确标注默认假设的数量 | 实际使用默认假设总数 | 100% | 自动 |
...
| 参数准确率 | 关键参数正确的数量 | 应传关键参数总数 | ≥95%（初始值待校准） | 自动 |
| 工具结果利用率 | 输出中正确使用工具结果的项数 | 工具返回可用结果总项数 | ≥90%（初始值待校准） | 半自动 |
| 无效调用率 | 重复、无关、缺关键参数调用数 | 总调用数 | ≤5%（初始值待校准） | 自动 |
| 工具调用成功率 | 成功返回并被正确处理的调用数 | 总调用数 | ≥90%（初始值待校准） | 自动 |
...
| 指标 | 分子 | 分母 | 通过阈值 | 评估方式 |
|---|---:|---:|---:|---|
| 上下文保持准确率 | 正确保持上下文变量的数量 | 应保持上下文变量总数 | ≥95%（初始值待校准） | 自动 |
| 状态更新正确率 | 新信息正确覆盖旧假设的数量 | 需要更新的状态项总数 | ≥95%（初始值待校准） | 半自动 |
| 行程冲突率 | 出现时间/地点/重复安排冲突的行程数 | 总行程数 | ≤5%（初始值待校准） | 半自动 |
| 多轮偏好响应率 | 用户修改偏好后方案正确调整的用例数 | 多轮修改用例数 | ≥90%（初始值待校准） | 人工 |
...
| 降级策略正确率 | 采取正确降级/替代策略的用例数 | 异常用例总数 | ≥90%（初始值待校准） | 人工 |
| 用户告知完整率 | 明确告知异常原因和下一步的用例数 | 异常用例总数 | ≥90%（初始值待校准） | 半自动 |
| 异常后任务完成率 | 异常恢复后仍输出可用方案的用例数 | 异常用例总数 | ≥85%（初始值待校准） | 人工 |
...
| 指标 | 分子 | 分母 | 通过阈值 | 评估方式 |
|---|---:|---:|---:|---|
| 事实可验证率 | 有工具来源或明确标记估算的事实项数 | 输出事实项总数 | ≥95%（初始值待校准） | 半自动 |
| 景点真实性准确率 | 真实且位于杭州/周边合理范围的景点数 | 推荐景点总数 | 100% | 半自动 |
| 营业/预约信息准确率 | 与来源一致的营业/预约信息数 | 输出营业/预约信息总数 | ≥95%（初始值待校准） | 半自动 |
| 餐厅真实性准确率 | 真实且位置匹配的餐厅数 | 推荐餐厅总数 | ≥95%（初始值待校准） | 半自动 |
| 严重幻觉数 | 不存在景点/餐厅、虚假预订、虚假票务等P0问题数 | 总用例数 | 0 | 半自动 |
...
| 指标 | 分子 | 分母 | 通过阈值 | 评估方式 |
|---|---:|---:|---:|---|
| 输出结构完整率 | 覆盖必要输出模块数 | 必要输出模块总数 | ≥90%（初始值待校准） | 半自动 |
| 行程可执行率 | 时间、地点、交通顺序均可执行的行程数 | 总行程数 | ≥90%（初始值待校准） | 人工 |
| 个性化匹配率 | 体现用户偏好/约束的安排数 | 应体现偏好/约束总数 | ≥85%（初始值待校准） | 人工 |
| 风险提示覆盖率 | 覆盖天气、预约、人流、票务等风险项数 | 应提示风险项总数 | ≥85%（初始值待校准） | 半自动 |
| 可读性评分 | 人工1-5分评分 | 评审人数 | 平均≥4分（初始值待校准） | 人工 |
...
| 端到端完成耗时P95 | 从用户输入到最终方案输出 | ≤30秒（初始值待校准） | 自动 |
| 工具调用次数 | 单次完整规划任务调用总数 | ≤10次（初始值待校准） | 自动 |
| 工具失败重试次数 | 同一工具同一参数重试次数 | ≤2次（初始值待校准） | 自动 |
| 总token消耗 | 输入+输出+中间推理摘要token | ≤12000 tokens（初始值待校准） | 自动 |
| 无效调用率 | 无效工具调用数 / 总工具调用数 | ≤5%（初始值待校准） | 自动 |
...
该输入用于主链路回归，重点覆盖意图理解、信息补全、任务拆解、工具调用、状态管理、异常恢复、幻觉检测、最终输出、性能与成本的端到端表现。

### 13.2 变体维度表

| 变体维度 | 变体示例 | 测试目的 |
|---|---|---|
| 时间表达 | “这周六日” / “9月第一个周末” / “后天开始玩两天” / “国庆后第一个周末” | 相对日期解析鲁棒性，验证不同时间表达能否标准化为具体日期 |
| 目的地 | “成都” / “大理” / “上海迪士尼+市区” / “苏州+杭州” | 不同城市、多目的地、景区+市区组合处理能力 |
| 天数 | “三天两晚” / “一日游” / “五天四晚” / “半天转机游” | 行程规模伸缩能力，验证任务拆解粒度和输出结构适配 |
| 偏好明确度 | “带3岁娃，预算3000，高铁” / “情侣轻松游，想拍照” / “户外徒步，预算不限” / “随便安排” | 信息充足与极度模糊输入下的主动澄清、个性化匹配和默认假设处理 |
| 约束冲突 | “预算500住五星酒店” / “一天逛10个景点” / “带老人但想特种兵路线” / “下雨也要全户外徒步” | 异常恢复、冲突提示、替代方案能力 |
| 模糊/歧义 | “去西湖玩玩” / “安排个周末放松” / “想去江南玩两天” / “找个适合拍照的地方” | 意图理解边界、目的地澄清、信息补全能力 |
...
1. 每个变体维度至少被覆盖2次。
2. 每个特殊路径至少被覆盖2次，包括主动澄清、异常恢复、幻觉检测回退、推理质量回退、性能成本压力路径。
3. 硬约束路径必须覆盖，包括相对日期解析、P0幻觉、虚假预订、严重约束冲突。
4. 核心输入每轮端到端回归必跑；变体子集可按版本风险选择全量或抽样执行。
5. 若某个变体在生产bad case中出现，应加入下一版固定回归子集。

### 13.4 变体标注模板

每条变体必须标注预期通过节点和预期触发特殊路径。

| 变体ID | 输入 | 覆盖维度 | 预期通过的节点 | 预期触发的特殊路径 | 主要断言 |
|---|---|---|---|---|---|
| V-001 | 这周六日去杭州玩两天 | 时间表达 | 1、3、5、6、8、9 | 相对日期解析 | 日期必须按评测基准日解析，不能误解为下周末 |
| V-002 | 带3岁娃，预算3000，高铁，帮我安排杭州两日游 | 偏好明确度 | 1-10 | 个性化路径 | 方案应亲子友好，节奏轻松，预算和高铁约束被保留 |
| V-003 | 预算500住西湖边五星酒店，杭州两天 | 约束冲突 | 1、2、3、4、7、9 | 异常恢复 | 必须提示预算与五星酒店冲突，并给出替代方案 |
| V-004 | 一天逛10个杭州景点 | 约束冲突 | 1、2、3、4、7、9 | 异常恢复 | 必须指出不可执行，压缩为合理路线或要求用户取舍 |
| V-005 | 去西湖玩玩 | 模糊/歧义 | 1、2、3、9 | 主动澄清 | 应追问出发时间、天数、预算、偏好，不能直接生成确定两日游 |
| V-006 | 上海迪士尼+市区两日游 | 目的地 | 1、3、5、6、8、9 | 多目的地/区域组合 | 应识别为上海目的地，不能套用杭州方案 |
| V-007 | 后天开始去成都玩三天两晚 | 时间表达+目的地+天数 | 1、3、5、6、8、9 | 日期解析+规模伸缩 | 应解析后天日期，输出三天两晚成都方案 |
| V-008 | 下雨也要全户外徒步，杭州两日游 | 约束冲突 | 1、3、4、5、7、9 | 天气异常恢复 | 应提示安全风险，并提供户外压缩或室内替代方案 |
