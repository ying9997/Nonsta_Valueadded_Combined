...
业务签批前，干跑配置暂跟 [`SIMULATED-BUSINESS-RULES.md`](SIMULATED-BUSINESS-RULES.md)；签批后以业务勾选覆盖。**先规则后干跑。**

| 节点 | 当前缺什么 | 规则来源 | 应写入位置 | 用哪条用例验收 |
|------|------------|----------|------------|----------------|
| validate-input | 只认库内 `OSF6V1603/1841` | `OW01V1602` + VASC `VASC202411192246131` | `nodes/validate-input.ts` | P-001 不得 `invalid_input` |
| match-template | 无 Top1 场景；易撞 5/6/11 | `2.1` 场景定义 | 新建 Top1 场景索引 | P-001～P-004 `sceneKey` |
| context-bind（缺） | 异常入口仍可能把单号/仓库当追问槽 | `2.1` A 层免追问 | 新逻辑或 check 前置 | P-001～P-003：`missingFields` 不得含这些 |
| extract / check-completeness | 旧字段或编造六必填 | 模拟必填三件：操作说明 / 商品和标签对应关系 / 标签文件 | Top1 `SCENARIO_FIELDS` | P-001（可选缺失不追问）；P-002 / P-003 |
