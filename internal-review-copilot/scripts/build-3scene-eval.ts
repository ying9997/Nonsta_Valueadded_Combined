/**
 * Round 2: score cleaned details → L1-L3 stratified samples + optional L4 real LLM.
 *
 * Usage:
 *   npx tsx internal-review-copilot/scripts/build-3scene-eval.ts \
 *     --input _runs/20260908_3scene_build_v2/details.json \
 *     --out _runs/20260908_3scene_eval_v2
 *
 *   # L4 only (after L1-L3 already scored; reuses scored buckets from same run):
 *   npx tsx internal-review-copilot/scripts/build-3scene-eval.ts \
 *     --input _runs/20260908_3scene_build_v2/details.json \
 *     --out _runs/20260908_3scene_eval_v2 \
 *     --with-l4
...
> - 理由？ ___`;
  ...
  // renderTrace already has a judgment block; replace tail if L4 custom, else append context before it
...
function promoteAttachmentsForL4(detail: JsonRecord, missingAttachments: string[]): JsonRecord {
  ...
  // Default LF if still nothing and L3 was attachment-gated
...
/** Append constructed boost to RD only (keeps original BEOR/RD; marks meta). */
...
function renderL4Extra(result: PipelineResult, elapsedMs: number): string {
  ...
  lines.push(`- token: prompt=n/a completion=n/a（客户端未返回 usage）`);
  ...
  lines.push("");
  lines.push("### 安全检查");
  lines.push(`- looksInvented: ${llm.error?.includes("编造") ? `检出 — ${llm.error}` : "调用路径内已校验；若 error 含编造则失败"}`);
  lines.push(`- assertNoForbidden: ${llm.error?.includes("禁止表述") ? `失败 — ${llm.error}` : "调用路径内已校验"}`);
  lines.push("");
  return lines.join("\n");
}
...
async function main(): Promise<void> {
  ...
  const inputPath = resolve(projectRoot, arg("input") || "_runs/20260908_3scene_build_v2/details.json");
  const outDir = resolve(projectRoot, arg("out") || "_runs/20260908_3scene_eval_v2");
  ...
  console.log(`details: ${allDetails.length} total, ${cleanDetails.length} clean (qualityFlag empty)`);
  ...
  for (const detail of cleanDetails) {
    ...
    if (!result) continue;
    const meta = asRecord(detail._caseMeta);
    const expectedScene = (asText(meta.expectedSceneAlias) as SceneCol) || "非三场景";
    const conversationAvailable = Boolean(
      detail.conversationAvailable ?? meta.conversationAvailable,
    );
    ...
    scored.push({
      ...
      expectedScene: ["F-001", "A", "B"].includes(expectedScene) ? expectedScene : "非三场景",
  ...
  const scenes: SceneCol[] = ["F-001", "A", "B", "非三场景"];
  ...
  // L1-L3 samples (2 per scene × exit)
  const l1l2l3Exits: ExitRow[] = ["L1", "L2-unsupported", "L2-ambiguous", "L3"];
  const samplesL123: ScoredRow[] = [];
  for (const sc of ["F-001", "A", "B"] as SceneCol[]) {
    for (const exit of l1l2l3Exits) {
      const pool = buckets.get(cellKey(sc, exit)) || [];
      samplesL123.push(...pool.slice(0, SAMPLE_L1L2L3));
    }
  }

  writeFileSync(
    resolve(outDir, "samples-l1l2l3.jsonl"),
    samplesL123
      .map((row) =>
        JSON.stringify({
          orderNo: row.orderNo,
          ...
          hasEb: asArray(asRecord(row.detail._caseMeta).ebNos).length > 0,
        }),
      )
      .concat([""])
      .join("\n"),
  ...
  const indexRows: Array<{ orderNo: string; result: PipelineResult; exit: string; scene: string }> = [];
  for (const row of samplesL123) {
    ...
    indexRows.push({ orderNo: row.orderNo, result: row.result, exit: row.exit, scene: row.expectedScene });
  }
  const indexLines = [
    "# Trace Index（L1-L3）",
    "",
    `- 样本数: ${samplesL123.length}`,
    "",
    "| # | VASC | expectedScene | exit | outputPath | decision | conv | trace |",
    "|---|------|---------------|------|------------|----------|------|-------|",
  ];
  indexRows.forEach((r, i) => {
    const row = samplesL123[i];
    indexLines.push(
      `| ${i + 1} | ${r.orderNo} | ${r.scene} | ${r.exit} | ${r.result.outputPath} | ${r.result.matchResult?.decision || "-"} | ${row.conversationAvailable ? "Y" : "N"} | [trace](${r.orderNo}.trace.md) |`,
    );
  });
    ...
    `${JSON.stringify(
      {
        ...
        sampleL1L2L3: samplesL123.length,
  ...
  if (!withL4) {
    console.log("skip L4 (pass --with-l4 to run real LLM)");
    return;
  }
  ...
  const naturalL4 = scored.filter((r) => r.exit === "L4");
  const l4Candidates: Array<{ row: ScoredRow; detail: JsonRecord; source: "natural" | "constructed_from_l3" }> =
    naturalL4.slice(0, L4_TARGET).map((row) => ({
      row,
      detail: row.detail,
      source: "natural" as const,
    }));

  if (l4Candidates.length < L4_MIN) {
    const l3Pool = scored.filter((r) => r.exit === "L3");
    for (const row of l3Pool) {
      if (l4Candidates.length >= L4_TARGET) break;
      if (l4Candidates.some((c) => c.row.orderNo === row.orderNo)) continue;
      const missing = row.result.missingAttachments || [];
      const promoted = promoteAttachmentsForL4(row.detail, missing);
      const check = await runPipeline(promoted, { skipLlm: true });
  ...
  // Coverage constructions when natural+L3 < 3: promote attachments (+ light scene-signal boost if still blocked at match).
  if (l4Candidates.length < L4_MIN) {
    const boostByScene: Record<string, string> = {
      "F-001": "请按尺重/标签辨识后换标上架到新入库单，并关闭异常单。",
      A: "包裹条码正常但商品条码异常，请换商品标签后上架到新入库单，并关闭异常单。",
      B: "请对指定商品拍照暂存，等客户确认后再处理，不要直接上架。",
    };
    const used = new Set(l4Candidates.map((c) => c.row.orderNo));
    const pool = [...scored].sort((a, b) => {
      const rank = (e: ExitRow) =>
        e === "L3" ? 0 : e === "L2-ambiguous" ? 1 : e === "L2-unsupported" ? 2 : 3;
      return rank(a.exit) - rank(b.exit);
    });
    for (const row of pool) {
      if (l4Candidates.length >= L4_MIN) break;
      if (used.has(row.orderNo)) continue;
      if (!["F-001", "A", "B"].includes(row.expectedScene)) continue;
      ...
      if (!check || check.outputPath !== "sop_generated") {
        const boost = boostByScene[row.expectedScene] || "";
        detail = boostRequirementText(detail, boost);
        const meta = asRecord(detail._caseMeta);
        meta.constructedIntentBoost = boost;
  ...
  console.log(`L4 candidates: ${l4Candidates.length} (natural=${naturalL4.length})`);
  ...
  for (const cand of l4Candidates) {
    ...
    // Inject richer LLM section before conclusion if present
    ...
    console.log(
      `L4 ${row.orderNo} source=${cand.source} path=${result.outputPath} err=${result.llm?.error || "null"} ${elapsedMs}ms`,
    );
  }

  writeFileSync(resolve(outDir, "samples-l4.jsonl"), `${sampleL4Lines.join("\n")}${sampleL4Lines.length ? "\n" : ""}`, "utf8");
  ...
  console.log(`L4 done: ${l4Results.length} traces → ${l4Dir}`);
}
