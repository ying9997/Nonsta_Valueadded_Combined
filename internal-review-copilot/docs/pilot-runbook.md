...
| 飞书 App ID | `cli_aa2a76198a7adcb3`（增值咨询；secret 本机 `.env`，不上 git / 未授权不上 40） |
| 试点群 `chat_id` | `oc_80b07f38ed6833df3787a97a496f1097` |
| 目录策略 | 方案 A（本机真源）→ E2E 跑通后再考虑方案 B staging → 仅 40 新建 |
| 40 门禁 | 本机（或本机连飞书）端到端跑通且现网隔离；见工作区 canonical §9 |
